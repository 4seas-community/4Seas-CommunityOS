...
// node_modules/.pnpm/@opennextjs+cloudflare@1.20.6_next@15.5.25_@types+node@24.13.6_react-dom@19.3.0_react@1_487fa4d62c8b1a4bb897084ded122d1d/node_modules/@opennextjs/cloudflare/dist/api/durable-objects/queue.js
...
var DOQueueHandler = class extends DurableObject {
    ...
    try {
      ...
      response = await this.service.fetch(`${protocol}://${host}${url}`, {
        ...
        headers: {
          ...
          "x-prerender-revalidate": "bff366be4eeb1d576962ea0f50284779",
          "x-isr": "1"
        },
  ...
  async addToFailedState(msg) {
    ...
    if (!this.disableSQLite) {
      this.sql.exec("INSERT OR REPLACE INTO failed_state (id, data, buildId) VALUES (?, ?, ?)", msg.MessageDeduplicationId, JSON.stringify(updatedFailedState), "JRDdm8Ewg5TxuV8aZbAzs");
    }
    await this.addAlarm();
  }
  async addAlarm() {
  ...
  async initState() {
    ...
    this.sql.exec("DELETE FROM failed_state WHERE buildId != ?", "JRDdm8Ewg5TxuV8aZbAzs");
    this.sql.exec("DELETE FROM sync WHERE buildId != ?", "JRDdm8Ewg5TxuV8aZbAzs");
